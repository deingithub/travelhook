...
def scotty_journey(jid):
    ...
    try:
        ...
        print(f"dbris journey hafas perl broke:\n{hafas.stdout} {hafas.stderr}")
        traceback.print_exc()


def scotty_stationboard(station, time):
    hafas = subprocess.run(
        ["json-hafas-stationboard.pl", "DBRIS", station, str(time)],
        capture_output=True,
    )
    status = {}
    try:
        ...
        print(f"dbris stationboard hafas perl broke:\n{hafas.stdout} {hafas.stderr}")
        traceback.print_exc()


def scotty_stopfinder(station):
    hafas = subprocess.run(
        ["json-hafas-dbris-stopfinder.pl", station],
        capture_output=True,
    )
    status = {}
    try:
        result = json.loads(hafas.stdout)
        if "stops" in result:
            # for some reason dbris sometimes doesn't report these? what???
            result["stops"] = [
                stop for stop in result["stops"] if stop["eva"] and stop["name"]
            ]
        return result
    except:  # pylint: disable=bare-except
        print(f"dbris stopfinder hafas perl broke:\n{hafas.stdout} {hafas.stderr}")
        traceback.print_exc()
    ...
    async def select_destination(self, ia, select):
        ...
        try:
            link = (
                ...
                + f"/0/{self.selected_origin['sched_dep'] * 1000}/?jid="
        ...
        if travel_token := DB.User.find(discord_id=ia.user.id).token_travel:
            api_payload = {
                "token": travel_token,
                "train": {
                    "type": self.selected_train["type"] or "",
                    "line": self.selected_train["line"] or "",
                    "no": self.selected_train["number"] or "1",
                },
                "fromStation": {
                    "name": self.selected_origin["name"],
                    "scheduledTime": departure.timestamp(),
                    "realTime": (
                        self.selected_train["realtime"] or departure.timestamp()
                    ),
                },
                "toStation": {
            ...
            async with ClientSession() as session:
                async with session.post(
                    ...
                    if data["success"]:
                        await ia.edit_original_response(
                            content=f"## [manual trip imported to travelynx]({config['travelynx_instance']}/journey/{data['id']})",
                            view=self,
                        )
                    else:
                        await ia.edit_original_response(
                            content=f"## travelynx import failed: {data['error']}",
                            view=self,
                        )
...
async def checkin(
    ...
    "perform a manual checkin from a station covered by DB HAFAS"
    ...
    await ia.response.defer(ephemeral=True)
    await ia.edit_original_response(
        content=f"### DB HAFAS-ish manual check-in",
